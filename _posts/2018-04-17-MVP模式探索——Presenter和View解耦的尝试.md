...
ContentProvider大家应该多多少少都有接触过把，我们把这个中间件看成一个ContentProvider即可，这个这个中间件提供一个注册View和Presneter关系的方法，然后提供一个可以发送一个特定Action的方法，这个Action里面有一个发送者的class、接收者class还有一个code和一些参数，这个code应着接收者里某个方法注解上的code，当中间件找到发送者的class和接收者class的关系后，然后根据Action里面的code决定执行接收者的哪个方法。这所有的逻辑都在中间件里面完成，无需外部实现，View和Presenter的关系就变得更模糊了。
